...
- **Lógica SQL:**
    ...
    SELECT *
    FROM sales.fact_sales
    WHERE order_date BETWEEN DATE_TRUNC('halfyear', CURRENT_DATE) AND CURRENT_DATE;

    -- Ventas del 'Semestre Pasado'
    SELECT *
    FROM sales.fact_sales
    WHERE order_date BETWEEN DATE_TRUNC('halfyear', CURRENT_DATE - INTERVAL '6 months') AND DATE_TRUNC('halfyear', CURRENT_DATE) - INTERVAL '1 day';
    ```

    ***Nota:*** Para extraer partes de fechas, PostgreSQL soporta `EXTRACT(part FROM date_expression)` además de `DATE_PART`. `DATE_TRUNC('quarter', date)` y `DATE_TRUNC('halfyear', date)` son funciones útiles en PostgreSQL para truncar fechas al inicio del trimestre o semestre, respectivamente. `INTERVAL` se utiliza para operaciones de resta de tiempo.
